...
class MessageParser {
	 ...
	 */
	static String bodyParse(Part p, ArrayList<String> files, ArrayList<String> medias) throws IOException,
			MessagingException {
	...
	private static String traitementMultipart(Part p, ArrayList<String> files, ArrayList<String> medias)
	...
	private static String traitementPart(ArrayList<String> files, ArrayList<String> medias, Multipart mp, String text) throws MessagingException, IOException {
		for (int i = 0; i < mp.getCount(); i++) {
			Part bp = mp.getBodyPart(i);
			if (bp.isMimeType("text/plain")) { //texte
				if (text == null)text = bodyParse(bp, files, medias);
				continue;
	...
	private static String traitementImage(Part p, ArrayList<String> files, ArrayList<String> medias)
	...
	private static String traitementPieceJointe(Part p, ArrayList<String> files, ArrayList<String> medias)
			throws IOException, MessagingException, FileNotFoundException {
		Multipart mp = ((Multipart)p.getContent());
		String s = "";
		for (int i = 0; i < mp.getCount(); i++) {
	...
	private static void createFichier(ArrayList<String> files, BodyPart bp, String path)
	...
	static Content messageToContent(Message message) throws IOException, MessagingException{
